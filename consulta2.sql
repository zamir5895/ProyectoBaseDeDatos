...
-- ordenada por el año de manera ascendente.
SELECT
...
FROM (
         ...
         ON c.dniPaciente = per.dni
             ...
             ON c.dniPaciente = citas_paciente.dniPaciente
             ...
             ON c2.idConsultorio = citas_por_año.idConsultorio
             AND EXTRACT(YEAR FROM c2.fecha) = citas_por_año.year
             GROUP BY
             c2.idConsultorio,
             EXTRACT(YEAR FROM c2.fecha)
             ) AS promedio_citas
             ON c.idConsultorio = promedio_citas.idConsultorio
             AND EXTRACT(YEAR FROM c.fecha) = promedio_citas.year
         WHERE
             citas_paciente.num_citas > promedio_citas.prom_citas
     ) AS consultorio_info
GROUP BY
    consultorio_info.idConsultorio,
    consultorio_info.year
